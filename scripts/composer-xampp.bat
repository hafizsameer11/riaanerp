...
REM Usage (from repo root):  scripts\composer-xampp.bat install
setlocal
...
if not exist "%XAMPP_PHP%" (
  echo Edit scripts\composer-xampp.bat: XAMPP_PHP not found at %XAMPP_PHP%
  exit /b 1
)
if not exist "%COMPOSER_PHAR%" (
  echo Edit scripts\composer-xampp.bat: composer.phar not found at %COMPOSER_PHAR%
  exit /b 1
)
pushd "%~dp0.."
