<?php
/**
 * NAS Email Parser - Processes ALL emails (SEEN + UNSEEN) and deletes after storage (POP behavior)
 * 
 ...
 * 
 * This script:
 * 1. Connects to NAS email account
 ...
 * 
 * Usage: php7.4 read_nas_emails.php
 */
...
// Source type
$source = 'NAS';
$logFile = $scriptDir . '/logs/nas_log.txt';
...
foreach ($allEmails as $emailNo) {
    ...
    try {
        ...
        // NAS Email Subject Format: "Wimico NAS [156.38.150.190] Network backup - Wimico_USA successful on Wimico"
        // Extract status from subject (successful, failed, error, etc.)
        // Also check body if subject doesn't have status
        if (preg_match('/\b(successful|success|failed|error|warning|complete)\b/i', $subject, $matches)) {
            $statusMatch = strtolower($matches[1]);
            if (in_array($statusMatch, ['successful', 'success', 'complete'])) {
                $status = 'Success';
            } elseif ($statusMatch === 'failed') {
                $status = 'Error';
            } elseif ($statusMatch === 'warning') {
                $status = 'Warning';
            } else {
                $status = 'Error';
            }
        } elseif (preg_match('/\b(successful|success|failed|error|warning|complete)\b/i', $bodyPlain, $matches)) {
        ...
        // Pattern: "Network backup - DeviceName successful" or "Network backup - DeviceName"
        if (preg_match('/Network\s+backup\s+-\s+([^\s]+)/i', $subject, $matches)) {
            $deviceType = trim($matches[1]);
        } elseif (preg_match('/Backup\s+Task[:\s]+([^\s<]+)/i', $bodyPlain, $matches)) {
            // Extract from body, but stop at HTML tags or newlines
            $deviceType = trim($matches[1]);
            // Clean up any extra text
            $deviceType = preg_replace('/\s*is\s+now\s+complete.*$/i', '', $deviceType);
            $deviceType = preg_replace('/<[^>]+>/', '', $deviceType);
            $deviceType = trim($deviceType);
        }
        
        // Extract backup date from email date or body
        if (!empty($date)) {
            $backupDate = date('Y-m-d H:i:s', strtotime($date));
        } elseif (preg_match('/Start\s+Time[:\s]+(?:Mon|Tue|Wed|Thu|Fri|Sat|Sun)[,\s]+(\d{1,2})\s+(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\s+(\d{4})\s+(\d{1,2}):(\d{2}):(\d{2})/i', $body, $matches)) {
            $monthNames = ['jan' => '01', 'feb' => '02', 'mar' => '03', 'apr' => '04', 'may' => '05', 'jun' => '06',
                          'jul' => '07', 'aug' => '08', 'sep' => '09', 'oct' => '10', 'nov' => '11', 'dec' => '12'];
            $month = strtolower(substr($matches[2], 0, 3));
        ...
        // Extract backup size from NAS-specific format
        // PRIORITY: "Increased Target Size" is the primary field to extract (e.g., "Increased Target Size: 410.9 MB")
        if (preg_match('/Increased\s+Target\s+Size[:\s]+([\d,\.]+\s*(?:GB|MB|KB|TB|B))/i', $body, $matches)) {
            $backupSize = trim($matches[1]);
        } elseif (preg_match('/New\s+Shared\s+Folder\s+Data\s+Size[:\s]+([\d,\.]+\s*(?:GB|MB|KB|TB|B))/i', $body, $matches)) {
            $backupSize = trim($matches[1]);
        } elseif (preg_match('/Total\s+Source\s+Size[:\s]*-?\s*Shared\s+folder[:\s]+([\d,\.]+\s*(?:GB|MB|KB|TB|B))/i', $body, $matches)) {
            $backupSize = trim($matches[1]);
        } elseif (preg_match('/Shared\s+folder[:\s]+([\d,\.]+\s*(?:GB|MB|KB|TB|B))/i', $body, $matches)) {
            $backupSize = trim($matches[1]);
        } elseif (preg_match('/Total\s+Source\s+Size[:\s]+([\d,\.]+\s*(?:GB|MB|KB|TB|B))/i', $body, $matches)) {
            $backupSize = trim($matches[1]);
        }
        
        // Keep backup size as-is from email (preserve comma or dot as shown in original email)
        // No conversion - store actual value as it appears: "410,9 MB" stays "410,9 MB"
        ...
        if ($isBackupEmail && $emailProcessedSuccessfully) {
            ...
            // Primary duplicate check: source + subject + backup_date + status + backup_size
            if (!empty($backupDate)) {
...
logMessage("Backup source: $source (NAS mailbox)", $logFile);
logMessage("========================================", $logFile);
